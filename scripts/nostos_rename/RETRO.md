...
3b, 3c, 12, 14. Steps 1–5 are local and can be repeated. Steps 6–14 write to
...
| `hashfix.py` | the one post-tip PR: old hash cites → final ones, plus `docs/ci/retro/*.md` and the release notes |
...
and the cairn `main` is frozen (nothing lands until step 11).
...
1. **Titles.** `python3 $T/retro.py titles --repo $CAIRN --rules $T/rules.py` keeps the
   ...
   group). Polish the drafts in `$T/retro/titles.tsv`. **The user signs off.**
2. **Rewrite.** `python3 $T/rewrite.py --source $CAIRN --work $NOSTOS --rules $T/rules.py`.
   ...
   parents preserved, tags on tree-equal merges, the tip tree == rules applied
   to `git archive` of the old tip, and every commit's tree == rules(old tree).
   It takes about 25 s. Output goes to `$NOSTOS/.git/nostos-retro/`:
...
4. **Hashfix + retro docs.** `python3 $T/hashfix.py --work $NOSTOS`. Review
   ...
   which must print `verify: OK` again.
5. **Dry-run the replay.** `python3 $T/replay.py --repo $NOSTOS`. This prints every
...
6. **Backup.**
   ...
   ```
7. **Archive the old repo.** Run `gh repo rename cairn-archive --repo unfazed-dev/cairn --yes`,
...
8. **Create the new repo.** `gh repo create unfazed-dev/nostos --public`. Leave it
   empty: no README, license or .gitignore. Anything created now would take PR/issue #1.
9. **Actions off.** Run `gh api -X PUT repos/unfazed-dev/nostos/actions/permissions -F enabled=false`,
...
10. **Root.**
    ...
    ```
11. **Replay.** `ARXA_ALLOW_MAIN_PUSH=1 python3 $T/replay.py --repo $NOSTOS --apply`.
...
12. **Check.** 39 merged PRs in order, `git ls-remote origin main` == `git -C $NOSTOS rev-parse main`,
    `v0.1.0`/`v0.2.0` on their merges, and two releases marked "historical — no binaries".
13. **Actions on.** `gh api -X PUT repos/unfazed-dev/nostos/actions/permissions -F enabled=true -f allowed_actions=all`.
14. **Protect `main`.** Run the `gh api` call in `docs/ci/setup.md`. It has to come
    after the replay, because protection would refuse the replay's pushes.

Local layout (decision 2c): once step 12 checks out, `mv $CAIRN …/cairn-archive`.
