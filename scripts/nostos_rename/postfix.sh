...
#   2. dart format, the dirs ci.yml checks  (flutter job + atlet)
...
for t in cargo dart uniffi-bindgen-cs; do
  command -v "$t" >/dev/null || { echo "postfix: $t not on PATH" >&2; exit 1; }
done
TARGET=${CARGO_TARGET_DIR:-$W/sdk/nostos_dotnet/target}

# ponytail: no `pub get` — format needs no resolution, and pub get may rewrite pubspec.lock (out of scope)
dart_format() { (cd "$1" && shift && dart format "$@" 2>&1 | { grep -v -e 'Package resolution error' -e 'Failed to resolve package URI' || true; }); }

echo "== cargo fmt --all"
cargo fmt --all
echo "== dart format"
dart_format sdk/nostos_flutter lib test example/lib
dart_format apps/atlet/flutter lib test integration_test
...
echo "== changed: $(grep -c . <<<"$changed" || true) files ($(grep -c '\.rs$' <<<"$changed" || true) .rs, $(grep -c '\.dart$' <<<"$changed" || true) .dart, $(grep -c "^$CS\$" <<<"$changed" || true) bindings)"
