...
/// A single entry point: open a [Nostos] sync connection AND resolve
...
class NostosDatabase {
  ...
  static Future<NostosDatabase> direct({
    ...
    if (schema.tables.isEmpty) {
      throw ArgumentError.value(
        schema.tables,
        'schema.tables',
        'NostosDatabase.direct requires a declared schema with at least one '
            'table — there is no server to fetch one from',
      );
    }
    final nostos = await Nostos.direct(
  ...
  ///
  /// [NostosDatabase.supabase] rotates this for you. [NostosDatabase.direct] and
  /// [NostosDatabase.connect] are handed a token once, so THEIR callers must
