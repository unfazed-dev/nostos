...
PowerSync's server is Node.js, with a published replication-ingest rate of ~2–4k ops/sec and per-client sync of 2–20k ops/sec (no published aggregate fan-out figure). **Nostos's server is pure Rust (tokio + axum)**, parsing `pgoutput` via `pgwire-replication`, fanning out to thousands of concurrent WebSocket clients with per-connection backpressure, measured at 2,618,601 ops/sec aggregate fan-out @ 1k clients, 0.00% drops (median of 3 passes, 2026-09-02, eval-only; the pre-fix figure was 833,307). We publish continuous benchmarks and **make the benchmark repo public** so the claim is auditable — see [`benches/results/RESULTS.md`](../benches/results/RESULTS.md); no PowerSync figure is directly comparable to this aggregate number.
