...
`nostos_data(table_name, pk, payload, applied_lsn)` — where `payload` is
...
The 2026-07-13 redesign plan specified something else: replace `nostos_data` with
...
-- before: SCAN nostos_data

CREATE INDEX ix ON nostos_data(json_extract(payload,'$.title'))
  WHERE table_name='tasks';          -- partial + expression index

EXPLAIN QUERY PLAN SELECT * FROM tasks WHERE title='t42';
-- after:  SEARCH nostos_data USING INDEX ix (<expr>=?)
```
...
**The client read model is SQLite VIEWs over `nostos_data`. Materialized typed
tables are rejected — not deferred.**

When a query is measurably slow, the sanctioned fix is a **partial expression
index on `nostos_data`**, as above. Not a storage rewrite.
...
`UPSERT` into `nostos_data` and every reader sees them through the view, so there
...
an internal table (`nostos_outbox`) *would* execute. Views protect the documented
