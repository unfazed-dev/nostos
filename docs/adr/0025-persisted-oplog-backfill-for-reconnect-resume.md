...
1. **Persisted op-log** (`nostos_oplog` Postgres table) written at the `FanOutService::run`-loop chokepoint, batched per commit boundary; indexed `(tenant_id, lsn)` for replay + `(table_name, pk)` for compaction.
2. **Replay-on-reconnect** — on subscribe, if `client_epoch == server_epoch` and the client checkpoint is within the op-log window, replay `nostos_oplog` from the checkpoint to the fresh sink then atomically hand off to live fan-out. Delivers missed INSERTs/UPDATEs/DELETEs.
...
- **Negative — write-amplification:** every WAL event now also writes a `nostos_oplog` row. `make bench` must re-measure the 142k ops/sec moat claim; ship before/after or revert (Tier-5 precedent, `docs/ROADMAP.md`). Batched per-commit writes mitigate. **Measured 2026-08-05 (slice 6 closed):** real-PG harness `crates/nostos-infra/tests/e2e_pg_write_amp.rs` proves amplification is **exactly 1:1** (`amp=1.000`, 200 source WAL events → 200 `nostos_oplog` rows) with **`oplog_dropped=0`**. The fan-out-side op-log cost was already shown invisible on the eval bench (833,305→833,307, `RESULTS.md`); the real-PG INSERT is off-loop and does not regress the fan-out ceiling. See `RESULTS.md` for the honesty-contract framing (fan-out ceiling ≠ real-PG ingest; never compared).
