...
2. **Dead-letter columns** — `last_error TEXT` and `dead_lettered_at INTEGER` added to the `nostos_outbox` schema, plus an idempotent `migrate_outbox_dlq()` ALTER TABLE for pre-existing DBs. `mark_dead_letter_with_error` writes both columns with a fallback to the old `dlq=1`-only path if the columns are absent (pre-migration DB). Used by `watchWriteStatus` (ADR-0027).
