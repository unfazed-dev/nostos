...
3. **Snapshot-reconcile fallback** (nostos-domain `WireFrame` boundary marker + nostos-client `apply`) — a begin/end marker on snapshot delivery; the client removes local rows absent from a complete snapshot. Fixes the offline-delete orphan P0 for the snapshot path (long gap / first-connect / epoch-mismatch). This is the Replicache-Reset / PowerSync-checkpoint-reconcile semantics.
