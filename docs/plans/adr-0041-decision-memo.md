...
|---|---|---|---|
| 1 | Same fixture + assertions pass over `ws://` and `iroh://` — snapshot rows arrive, checkpoint advances, reconnect idempotent | verified 2026-08-27 (green run); structure re-verified 2026-08-29 | `crates/nostos-client/tests/iroh_ws_conformance.rs`: one shared `conformance_leg(url)` driven by `conformance_over_ws` and `conformance_over_iroh`. Run: `cargo test -p nostos-client --features iroh --test iroh_ws_conformance`. Not re-run tonight — branch tip unchanged since the green run (still `680852f`) |
...
| 6 | Merge cost onto today's main | verified 2026-08-29 | `git merge-tree --write-tree main spike/iroh-transport` probe: **one conflicting file** — `crates/nostos-client/src/client.rs` (spike's `SyncWs`/dial region vs main's `mem::take` + rustfmt commits). Mechanical resolution |
