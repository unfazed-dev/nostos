...
- **Track D — ADR-0041 ACCEPTED 2026-08-29** (merge `71fecbf`; decision memo `docs/plans/adr-0041-decision-memo.md`). Spike green: ws/iroh conformance parity re-run at `22bc108`, iroh off-default everywhere, iOS/Android build viability verified. **Accept-gated items (now tracked work, conditions in ADR-0041 §Acceptance):**
...
`arxa/kit/nostos` pins `nostos_flutter` by git SHA `ef3a40b` (on origin); `arxa/desktop` is
