...
- **A4 PARTIAL**: the rail is `make sdk-e2e tauri` (host slices always run) — 18 unit tests incl. a live spine round-trip (`nostos_state_live_round_trip_against_spine`), the offline CRDT round-trip + client-gate test, and the pre-connect honesty tests. REMAINING: porting the flutter adapter-conformance suite to an actual Tauri fixture APP (first task when a client adopts tauri).
