...
| # | decision | answer | why |
|---|---|---|---|
| 4 | `scripts/check.sh` areas | one area per `ci.yml` job (`lint-test`, `appwrite-function`, `atlet-cloud`, `e2e-pg`, `deny`, `sdk-e2e`, `flutter`, `benchmark`, `sdk-typecheck`) plus `all`; `atlet-cloud` requires the ignored real-cloud credentials and fails when absent | local green = CI green, by the same names; the hosted cloud gate must never pass by skipping a missing secret |
