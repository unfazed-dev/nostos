...
| # | decision | answer | why |
|---|---|---|---|
| 4 | `scripts/check.sh` areas | one area per `ci.yml` job (`lint-test`, `appwrite-function`, `atlet-cloud`, `atlet-web-cloud`, `atlet-server-cloud`, `e2e-pg`, `deny`, `sdk-e2e`, `flutter`, `benchmark`, `sdk-typecheck`) plus `all`; Atlet cloud areas require their toolchains and ignored real-cloud credentials and fail when any are absent | local green = CI green, by the same names; the hosted cloud gates must never pass by skipping a missing tool or secret |
