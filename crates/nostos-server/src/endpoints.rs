...
pub(crate) async fn metrics_handler(metrics: Arc<Metrics>, store: Arc<dyn SessionStore>) -> String {
    ...
    let last_pushed: String = metrics
        ...
        .map(|map| {
            map.iter().fold(String::new(), |mut acc, (account, lsn)| {
                ...
                let _ = std::fmt::Write::write_fmt(
                    &mut acc,
                    format_args!("nostos_push_last_lsn{{account=\"{escaped}\"}} {lsn}\n"),
                );
                acc
            })
        })
        .unwrap_or_default();
    format!(
        "# HELP nostos_events_matched_total Events whose predicate matched ≥1 session.\n\
         # TYPE nostos_events_matched_total counter\n\
         nostos_events_matched_total {matched}\n\
         # HELP nostos_events_delivered_total Events accepted by a session sink.\n\
         # TYPE nostos_events_delivered_total counter\n\
         nostos_events_delivered_total {delivered}\n\
         # HELP nostos_events_dropped_total Events dropped (full buffer / dedup / closed).\n\
         # TYPE nostos_events_dropped_total counter\n\
         nostos_events_dropped_total {dropped}\n\
         # HELP nostos_events_superseded_total Events that replaced a still-waiting frame for the same row in a sink's conflating overflow (ADR-0045). NOT loss — every frame is a complete row image, so the client converges from the newer one alone. Subtract from attempted alongside dropped when computing a drop rate from frame counts.\n\
         # TYPE nostos_events_superseded_total counter\n\
         nostos_events_superseded_total {superseded}\n\
         # HELP nostos_events_faulted_total Delivery tasks that faulted (panicked / cancelled) — a server fault, NOT slow-client backpressure. Kept distinct from nostos_events_dropped_total so a panic is never mis-attributed as a client drop in the 0%-drops figure. Alert on any increase.\n\
         # TYPE nostos_events_faulted_total counter\n\
         nostos_events_faulted_total {faulted}\n\
         # HELP nostos_live_sessions Current live sync sessions.\n\
         # TYPE nostos_live_sessions gauge\n\
         nostos_live_sessions {sessions}\n\
         # HELP nostos_slot_wal_status Replication-slot health gauge (0=healthy, 1=reserved, 2=lost, 3=recreated). 'lost' means silent-data-loss risk; alert on nostos_slot_recreated_total. ADR-0009.\n\
         # TYPE nostos_slot_wal_status gauge\n\
         nostos_slot_wal_status {slot_wal_status}\n\
         # HELP nostos_replication_lag_bytes Current WAL lsn minus slot restart_lsn, in bytes. 0 when slot is missing/unknown.\n\
         # TYPE nostos_replication_lag_bytes gauge\n\
         nostos_replication_lag_bytes {replication_lag_bytes}\n\
         # HELP nostos_slot_recreated_total Number of times the replication slot was dropped + re-created from a missing/lost state. Each increment is a potential silent-data-loss window; alert on any increase.\n\
         # TYPE nostos_slot_recreated_total counter\n\
         nostos_slot_recreated_total {slot_recreated_total}\n\
         # HELP nostos_oplog_dropped_total Op-log entries dropped (writer buffer full). The resume path falls back to snapshot-reconcile for the gap. ADR-0025.\n\
         # TYPE nostos_oplog_dropped_total counter\n\
         nostos_oplog_dropped_total {oplog_dropped}\n\
         # HELP nostos_oplog_flush_failed_total Op-log batch flushes that failed (PG error / connection lost). Batch lost; resume falls back to snapshot-reconcile. ADR-0025.\n\
         # TYPE nostos_oplog_flush_failed_total counter\n\
         nostos_oplog_flush_failed_total {oplog_flush_failed}\n\
         # HELP nostos_slot_epoch Monotonic epoch bumped on every replication-slot (re)creation. A client whose last-seen epoch differs must full-snapshot (cannot backfill from a recreated slot's dead lineage). ADR-0025.\n\
         # TYPE nostos_slot_epoch gauge\n\
         nostos_slot_epoch {slot_epoch}\n\
         # HELP nostos_oplog_compacted_rows_total Rows swept by op-log compaction (collapse duplicates to latest op per (table_name, pk) + age out rows past the retention window). ADR-0025 slice 5.\n\
         # TYPE nostos_oplog_compacted_rows_total counter\n\
         nostos_oplog_compacted_rows_total {oplog_compacted_rows}\n\
         # HELP nostos_push_enqueued_total Push doorbell hints enqueued (one per matched offline account; online accounts suppressed at enqueue time). ADR-0037.\n\
         # TYPE nostos_push_enqueued_total counter\n\
         nostos_push_enqueued_total {push_enqueued}\n\
         # HELP nostos_push_dropped_total Push hints dropped (bounded channel full / consumer gone). Doorbell semantics: a dropped hint loses nothing — the durable LSN checkpoint reconciles. ADR-0037.\n\
         # TYPE nostos_push_dropped_total counter\n\
         nostos_push_dropped_total {push_dropped}\n\
         # HELP nostos_push_sent_total Push sends the rails accepted (2xx). Last-mile delivery stays best-effort; the client's LSN ack is the proof. ADR-0037 plan 3.2.\n\
         # TYPE nostos_push_sent_total counter\n\
         nostos_push_sent_total {push_sent}\n\
         # HELP nostos_push_failed_total Push sends that failed terminally or exhausted their retry (rail fatal/transient). ADR-0037 plan 3.2.\n\
         # TYPE nostos_push_failed_total counter\n\
         nostos_push_failed_total {push_failed}\n\
         # HELP nostos_push_pruned_total Push-token rows pruned (rail reported the target gone, or the owner deregistered). ADR-0037 plan 3.2.\n\
         # TYPE nostos_push_pruned_total counter\n\
         nostos_push_pruned_total {push_pruned}\n\
         # HELP nostos_push_last_lsn Highest doorbell LSN pushed per account — correlate against session acked-LSN to see whether a doorbelled device actually caught up. ADR-0037 plan 3.2.\n\
         # TYPE nostos_push_last_lsn gauge\n\
         {last_pushed}",
